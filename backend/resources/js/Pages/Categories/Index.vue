<template>
	<div>
		<h1 class="mb-5 font-bold text-2xl">Categories</h1>
		...
		<div class="bg-white rounded-md shadow overflow-x-auto">
			<table class="w-full divide-y divide-gray-200" v-if="categories.data.length">
				...
				<tbody class="bg-white divide-y divide-gray-200">
					<tr v-for="(row, index) in categories.data" :key="index">
						...
						<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
							<inertia-link class="btn btn-primary mr-2" :href="route('categories.edit', row.id)">
		...
		<jet-pagination class="mt-6" :links="categories.links" />
	</div>
</template>
