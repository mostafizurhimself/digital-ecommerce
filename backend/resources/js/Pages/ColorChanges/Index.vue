<template>
	<div>
		<h1 class="mb-5 font-bold text-2xl">Change Color</h1>
		<div class="mb-4 flex justify-between items-center">
			...
			<a :href="route('download-excel.color-changes', searchQuery)" target="_blank" class="btn btn-purple">
		...
		<div class="bg-white rounded-md shadow overflow-x-auto">
			<table class="w-full divide-y divide-gray-200" v-if="colorChanges.data.length">
				...
				<tbody class="bg-white divide-y divide-gray-200">
					<tr v-for="(row, index) in colorChanges.data" :key="index">
						...
						<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
							<inertia-link class="btn btn-primary mr-2" :href="route('color-changes.show', row.id)">
								<i class="ti-eye"></i>
							</inertia-link>
						</td>
					</tr>
		...
		<jet-pagination class="mt-6" :links="colorChanges.links" />
	</div>
</template>
